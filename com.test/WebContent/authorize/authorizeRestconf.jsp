...
//connect to database
String sql = "SELECT userLevel from restaurantGroup where name=?";

PreparedStatement execStat1 = conn.prepareStatement("SELECT count(name) from restaurantgroup where userLevel = 1 ");
...
try {
   ...
   if(rs.next()) {
    if (action.equals("upgrade") && rs.getInt(1) == 0 && count <2) {
    	...
    	PreparedStatement execStat = conn.prepareStatement("UPDATE restaurantGroup SET userLevel = 1 WHERE name = ? ");
    	...
    	PreparedStatement execStat = conn.prepareStatement("UPDATE restaurantGroup SET userLevel = 0 WHERE name = ?");
		execStat.setString(1, name);
		execStat.executeUpdate();
		execStat.close();
    }
    ...
    <jsp:forward page="authorizeRest.jsp"/>
    <%
   }
